...
def import_raw_year(filename, year):
    ...
    df['Season'] = year
    df['adj_em'] = df['adj_o'].astype(float) - df['adj_d'].astype(float)
    return df
...
def clean_name(str_in):
    str_out =str_in.replace(".", "")
    str_out =str_out.replace(";", "")
    str_out =str_out.replace("Cal St","CS")
    str_out = ''.join([i for i in str_out if not i.isdigit()])
    return str_out.strip()
df['TeamName'] = df['TeamName'].apply(clean_name)
print(df.loc[df['Season']==2015]["TeamName"][0])
...
df.replace("Cal St. Bakersfield","CS Bakersfield", inplace = True)
...
df.replace("Texas Southern","TX Southern", inplace = True)
...
df=df[["rank", "TeamName","Season", "TeamID", "conference", "record", "adj_em", "adj_o", 
                "adj_o_rank", "adj_d", "adj_d_rank", "adj_tempo", "adj_tempo_rank", 
                "luck", "luck_rank", "sos_adj_em", "sos_adj_em_rank", "sos_adj_o",
                "sos_adj_o_rank","sos_adj_d", "sos_adj_d_rank", "nc_sos_adj_em", 
                "nc_sos_adj_em_rank"]]
...
print(df)
print(df.loc[(df['Season']==2015) & (df['TeamName']=="Kentucky")])

df.to_csv('data/Mkenpom2021.csv', index=False)